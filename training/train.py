...
def train_step(
    ...
    for batch in tqdm.tqdm(train_data):
        ...
        wandb.log(
            {
                "train/recon": training_recon / len(train_data),
                "train/kl": training_kl / len(train_data),
                "train/error": training_latent_error / len(train_data),
                "train/sparsity": loss[3].item(),
                "train/intervention_targets": loss[4].item(),
                "iter_idx": iter_idx,
            }
        )
        iter_idx += 1
    ...
    wandb.log(
        {
            "val/recon": val_recon / len(val_data),
            "val/kl": val_kl / len(val_data),
            "val/error": val_latent_error / len(val_data),
        }
    )
    return state, rng, iter_idx


def pretrain_step(state, model, rng, train_data, val_data, iter_idx):
    ...
    for batch in train_data:
        ...
        training_recon += loss[0].item()
        training_kl += loss[1].item()
    wandb.log({"train/recon", training_recon / len(train_data),
               "train/kl", training_kl / len(train_data)})
    ...
    for batch in val_data:
        ...
        val_recon += loss[0].item()
        val_kl += loss[1].item()
    wandb.log({"val/recon", val_recon / len(val_data),
               "val/kl", val_kl / len(val_data)})
